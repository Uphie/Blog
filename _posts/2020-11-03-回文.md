...
categories: [技术]
tags: [算法,双指针,链表,单链表,双链表,python,go]
math: true
toc: true
...
平分两段，其中一段翻转过来，与另一端比较。相同则是回文字符串。
```python
def check_palindrome(text: str) -> bool:
    ...
    if l % 2 == 0:
        # 偶数长
        return "".join(text[:l // 2]) == "".join(reversed(text[l // 2:]))
    # 奇数长
    return "".join(text[:l // 2]) == "".join(reversed(text[l // 2 + 1:]))
...
我们可以仿照 [平分2段](#平分2段) 的思路，取两段，一段翻转，然后两段比较是否相同即可。但是这需要找到这个单链表的中间的元素。此时可以利用前面文章 [查找单链表中间值](https://uphie.studio/2020-10-21/查找单链表中间值) 的方法来找到中间值。
