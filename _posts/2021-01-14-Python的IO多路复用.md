...
- `select`，支持类 Unix系统、Windows（只支持socket）
- `poll`，支持 Linux
- `epoll`，支持 Linux
- `kqueue`，支持 MacOS
- `devpoll`，支持 Linux
...
```
                ...
                |-----------------------------------|---------------------------------|
                |                                   |                                 |
SelectSelector(_BaseSelectorImpl)   _PollLikeSelector(_BaseSelectorImpl)  KqueueSelector(_BaseSelectorImpl)  
                                                    |
                                                    |
                        |---------------------------|---------------------------------|
            PollSelector(_PollLikeSelector)   EpollSelector(_PollLikeSelector)   DevpollSelector(_PollLikeSelector)

```

`BaseSelector` 是一个抽象类，定义抽象方法。
`_BaseSelectorImpl` 是对`BaseSelector`的基础实现，主要是管理文件描述符的注册、注销，以便底层IO多路复用模块进行监视。
...
简而言之，selector 是对各平台最佳 IO 多路复用模块的封装，将IO事件封装和管理，然后调用底层函数获取出就绪的文件描述符、事件和数据。
