...
categories: [技术]
tags: [数据结构,二叉树,二叉搜索树,二叉查找树,二叉排序树,go]
math: true
toc: true
---
...
由于它的性质，可以往树的最左侧找到这棵树上的最小值，往树的最右侧找到这棵树的最大值。查找一个值的平均时间复杂度为 $$O(logn)$$，最好的情况查找1次即可，最差的情况查找次数为树的高度（层次）。

但它有个缺点，不平衡，查找效率不稳定，极端情况下可以退化为一个链表。
...
因此，二叉搜索树不是平衡树，查找效率并不稳定。如果要求一个查找效率稳定的二叉搜索树就需要使用平衡二叉搜索树。
